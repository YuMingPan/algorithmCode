...
public class ListGraphSelfTest<V, E> extends Graph<V, E> {
	public ListGraphSelfTest() {}
	public ListGraphSelfTest(WeightManager<E> weightManager) {
		super(weightManager);
	}

	/**
	 * 顶点内部类
	 * @param <V> 泛型，如人，继而和边一起表达任何人之间的关系
	 * @param <E> 权值，可泛型
	 */
	private static class Vertex<V, E> {
		V value;
		// 拿到顶点的出入边(边和边之间没有顺序，即没有编号之说，所以用Set)
...
//	}
	...
	public void  dfsSelf(V begin) {
		Vertex start = vertices.get(begin);
		if(start == null) {
			return;
		}

		Stack<Vertex> stack = new Stack<>();
		Set<Vertex> vistedVertices = new HashSet();

		// 一开始就访问过start了，所以后续访问并不是一while就访问的
		stack.push(start);
		vistedVertices.add(start);

		while (!stack.isEmpty()){
			Vertex vertex = stack.pop();
			Set<Edge> outEdges = vertex.outEdges;
			for (Edge outEdge : outEdges) {
				if (vistedVertices.contains(outEdge.to)){
					continue;
				}

				stack.push(outEdge.from);
				stack.push(outEdge.to);
				vistedVertices.add(outEdge.to);
